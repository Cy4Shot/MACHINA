...
import com.machina.api.block.menu.slot.PlayerInvSlot;
	...
	public void invSlots(Inventory inv, int offset) {
		for (int l = 0; l < 3; ++l) {
			for (int j1 = 0; j1 < 9; ++j1) {
				this.addSlot(new PlayerInvSlot(inv, j1 + l * 9 + 9, 28 + j1 * 20, 83 + l * 20 + offset));
			}
		}

		for (int i1 = 0; i1 < 9; ++i1) {
			this.addSlot(new PlayerInvSlot(inv, i1, 28 + i1 * 20, 148 + offset));
		}
	}
