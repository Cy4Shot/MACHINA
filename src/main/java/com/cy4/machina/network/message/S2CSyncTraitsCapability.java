...
public class S2CSyncTraitsCapability implements IMachinaMessage {

	private final IPlanetDataCapability cap;
	private final ResourceLocation levelName;
	
	public S2CSyncTraitsCapability(IPlanetDataCapability cap, ResourceLocation levelName) {
		this.cap = cap;
	...
	@OnlyIn(Dist.CLIENT)
	private void handleClient(S2CSyncTraitsCapability msg, NetworkEvent.Context ctx) {
	...
	public static S2CSyncTraitsCapability decode(PacketBuffer buffer) {
		IPlanetDataCapability cap = new DefaultPlanetDataCapability();
		CapabilityPlanetData.Storage.deserialize(buffer.readNbt(), cap);
		return new S2CSyncTraitsCapability(cap, buffer.readResourceLocation());
	}
