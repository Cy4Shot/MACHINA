...
import com.machina.world.carver.CarverDecorator;
import com.machina.world.carver.DynamicDimensionCarver;
...
import com.machina.world.settings.DynamicStructureSettings;
...
public class DynamicDimensionChunkGenerator extends ChunkGenerator {
	...
	//@formatter:off
	public static final Codec<DynamicDimensionChunkGenerator> CODEC = RecordCodecBuilder.create(instance -> instance.group(
			RegistryLookupCodec.create(Registry.BIOME_REGISTRY).fieldOf("biomes").forGetter(DynamicDimensionChunkGenerator::getBiomeRegistry),
			Codec.INT.fieldOf("id").forGetter(DynamicDimensionChunkGenerator::getId)
		).apply(instance, DynamicDimensionChunkGenerator::new));
	//@formatter:on
...
//	private final INoiseGenerator surfaceNoise;
	...
	// Noise Settings
	public DynamicNoiseSettings noisesettings = DynamicNoiseSettings.OVERWORLD_TYPE;
	...
	public DynamicDimensionChunkGenerator(MinecraftServer server, RegistryKey<Dimension> key) {
		this(server.registryAccess().registryOrThrow(Registry.BIOME_REGISTRY), PlanetUtils.getIdDim(key));
		...
		if (this.cavesExist)
			this.carvers.add(DynamicDimensionCarver.create(this.caveChance, this.caveLength, this.caveThickness));
	...
	public DynamicDimensionChunkGenerator(Registry<Biome> biomes, int id) {
	...
	public void applyCarvers(long pSeed, BiomeManager pBiomeManager, IChunk pChunk, Carving pCarving) {
		...
		for (int a = 0; a < bitset.length(); ++a) {
			if (bitset.get(a)) {
				...
				CarverDecorator.decorateCavesAt(pChunk, pos, this);
			}
		}
