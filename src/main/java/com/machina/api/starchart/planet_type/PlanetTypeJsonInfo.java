...
public record PlanetTypeJsonInfo(String name, int iconY, int color, Shape shape, List<BiomePlacementJsonInfo> biomes,
		List<String> weathers, List<String> traits, String base) implements JsonInfo<PlanetType> {
	...
	public PlanetType cast() {
		...
		List<PlanetTrait> traits = traits().stream().map(x -> RegistryInit.TRAIT.get(ResourceLocation.parse(x)))
				.collect(Collectors.toList());

		HolderLookup<Block> block = BlockHelper.blockHolderLookup();
		BlockState base = BlockHelper.parseState(block, base());
		return new PlanetType(name, iconY, color, shape(), biomes, weathers, traits, base);
	}
}