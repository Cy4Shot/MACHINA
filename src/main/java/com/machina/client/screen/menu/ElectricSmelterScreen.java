	...
	protected void renderBg(@NotNull GuiGraphics gui, float pt, int mx, int my) {
		...
		if (entity.isLit()) {
			text = Component.literal(
					MUI.uistrs("electric_smelter.progress") + ": " + StringUtils.formatPercent(entity.getProgress())
							+ " (" + StringUtils.formatTicks(entity.ticksRemaining()) + ")");
			text2 = Component.literal(MUI.uistrs("electric_smelter.usage") + ": "
					+ StringUtils.formatPower(entity.getPowerRate()) + "/t");
		} else {
			if (!entity.hasRecipe()) {
				text = MUI.uistr("electric_smelter.no_input");
			} else if (!entity.meetsRequirements()) {
				text = Component.literal(MUI.uistrs("electric_smelter.no_power") + " ("
						+ StringUtils.formatPercent(entity.getProgress()) + ")");
				text2 = Component.literal(MUI.uistrs("electric_smelter.requires") + ": "
						+ StringUtils.formatPower(entity.getPowerRate()) + "/t");
