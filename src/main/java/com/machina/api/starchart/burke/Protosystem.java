...
public class Protosystem extends Object implements PhysicalConstants {
	...
	/**
	 * Planetary system contructor. Builds an accretion disc for the specified
	 * planet around the specified star. This has not been exercised to any
	 * significant degree, because moon formation doesn't seem to follow Dole's
	 * model quite as well.
	 * 
	 * @param s Primary for this system.
	...
	/**
	 * Searches the planetesimals already present in this system for a possible
	 * collision. Does not run any long-term simulation of orbits, doesn't try to
	 * eject bodies...
	 * 
	...
	public Protoplanet dist_planetary_masses() {
		...
		while (disc.dust_left) {
			...
			if (disc.dust_available(p0)) {
				p0.dust_density = DUST_DENSITY_COEFF * Math.sqrt(star.SM)
	...
	/**
	 * Accretes protoplanets from the dust disc in this system. This ought to work,
	...
	public Protoplanet dist_moon_masses() {
		...
		while (disc.dust_left) {
			p0 = new Protoplanet(disc.body_inner_bound, disc.body_outer_bound);
			if (disc.dust_available(p0)) {
				p0.dust_density = DUST_DENSITY_COEFF * Math.sqrt(planet.mass / SUN_MASS_IN_EARTH_MASSES)
						* Math.exp(-ALPHA * Math.pow(p0.a, (1.0 / N)));
