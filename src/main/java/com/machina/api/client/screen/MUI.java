...
public final class MUI {
	...
	public enum MuiSlot {
		...
		TICK(499, 93), TIME(499, 103), TEMP(499, 113), PLATE(499, 123), ROD(499, 133), CROSS_R(499, 143);
	...
	private static void renderCustomItemDecorations(GuiGraphics gui, ItemStack stack, int x, int y, boolean active) {
		if (!stack.isEmpty()) {
			...
			float f = mc.player == null ? 0.0F
					: mc.player.getCooldowns().getCooldownPercent(stack.getItem(),
	...
	public static TextureAtlasSprite getSprite(ResourceLocation spriteLocation) {
		return mc.getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(spriteLocation);
	}
