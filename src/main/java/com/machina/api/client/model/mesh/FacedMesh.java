...
@OnlyIn(Dist.CLIENT)
public record FacedMesh(Vector3f from, Vector3f to, Map<Direction, MeshFace> faces, boolean shade) implements IMesh {

    public static FacedMesh cube(Vector3f from, Vector3f to, String tex) {
        Material m = new Material(InventoryMenu.BLOCK_ATLAS, MachinaRL.create(tex));
    ...
    public static FacedMesh pipe(Vector3f from, Vector3f to, String tex, String tex_2) {
        Material m = new Material(InventoryMenu.BLOCK_ATLAS, MachinaRL.create(tex));
        Material m1 = new Material(InventoryMenu.BLOCK_ATLAS, MachinaRL.create(tex_2));
        Map<Direction, MeshFace> faces = Map.of( //@formatter:off
    ...
    public FacedMesh(Vector3f from, Vector3f to, Map<Direction, MeshFace> faces, boolean shade) {
        this.from = from;
        this.to = to;
        this.faces = faces;
        this.shade = shade;
    ...
    public float[] uvsByFace(Direction d) {
        ...
        return new float[]{0, 0, width / 16.0F, height / 16.0F};
    }
